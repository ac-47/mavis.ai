...
class WhisperState: NSObject, ObservableObject, AVAudioRecorderDelegate {
    ...
    let model = GenerativeModel(name: "gemini-1.5-flash-latest", , systemInstruction: ModelContent(role: "systm", parts: "You are MAVIS(My Average Very Intelligent System). your goal is to help user as an asistant. You have a witty british personality."),apiKey: // inpup api key)

    func sendMessage(promptText: String) async {
        if promptText.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
            messageLog += "Skiping generation: No prompt found.\n"
            return
        }
        
        messageLog += "Sending prompt to Gemini...\n"
        do {
            let response = try await model.generateContent(promptText)
            
            if let geminiResponse = response.text {
                messageLog += "MAVIS: \(geminiResponse)\n"
            } else {
                messageLog += "ERROR: Gemini response was empty.\n"
            }
        } catch {
...
//        messageLog += await WhisperContext.benchGgmlMulMat(nThreads: nThreads) + "\n"
    ...
    private func transcribeAudio(_ url: URL) async {
        ...
        do {
            ...
            messageLog += "You Said: \(text)\n"
            let response = await sendMessage(promptText: text)
        } catch {
            print(error.localizedDescription)
            messageLog += "\(error.localizedDescription)\n"
        }
